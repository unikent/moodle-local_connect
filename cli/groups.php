...
foreach (json_decode(file_get_contents('php://stdin')) as $c) {
    ...
    try {
        ...
        if (!$uid) {
          throw new moodle_exception('user not created yet?');
        }
        ...
        if ($c->isa == 'NEW') {
            ...
            if ($group === false) {
                throw new moodle_exception('Group does not exist');
            }
            ...
            if ($uid) {
                // If the user doesnt exist, their enrolments should have been wiped already
                // for now just remove the user, not the group
                $group = $DB->get_record('groups', array('id' => $c->moodle_group_id));
                if ($group) {
                    // cant find the group, must already not be there
